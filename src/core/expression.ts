 ...
 * the `ExpressionRef` class), since it allows for `T` to be a union
 ...
 */
export class ExpressionRef<T extends Type = any, Props extends object = {}> {
...
export interface ExpressionRef<T extends Type> extends Expression<T> {
  is: CheckBuilder<T>
}
